...
class PasswordGeneratorTest extends munit.FunSuite {
  ...
  test("Password must contains at least one special character") {
    val password = PasswordGenerator.generate(length = 10, specials = true, numbers = true, uppercases = true)
  ...
  test("Password must not contains special characters") {
    val password = PasswordGenerator.generate(10, specials = false, numbers = true, uppercases = true)
    val pattern = Pattern.compile("[!@#$%&*()_+=|<>?{}\\[\\]~-]")
  ...
  test("Password must contains only numbers") {
    val password = PasswordGenerator.generate(10, specials = false, numbers = true, uppercases = false)
  ...
  test("Password must contains only letters(camel case)") {
    val password = PasswordGenerator.generate(10, specials = false, numbers = false, uppercases = true)
